...
struct Grammar {
    ...
    // %define api.value.automove -- wrap $N references in std::move(...)
    // so move-only types work in semantic values.  C++ only.
    bool api_value_automove = false;
    // %define api.token.constructor -- user calls parser::make_NAME(value)
    // factories instead of returning the token-kind int.  C++ only.
    bool api_token_constructor = false;
...
public:
    ...
    Token next_grammar() {
        ...
        if (c == '<') {
            advance();
            // Bison's tag content can contain template angle-brackets
            // (std::unique_ptr<int> etc.); track nesting and read until
            // the matching outer '>'.  Also accept "*" / "%empty" /
            // "*"-style as bison does, but the simple loop suffices.
            string tag;
            int depth = 1;
            while (pos_ < src_.size() && depth > 0) {
                char ch = src_[pos_];
                if (ch == '<') { depth++; tag.push_back(ch); advance(); }
                else if (ch == '>') {
                    depth--;
                    if (depth == 0) { advance(); break; }
                    tag.push_back(ch); advance();
                }
                else { tag.push_back(ch); advance(); }
            }
            if (depth != 0) fatalf("unterminated <tag> at line {}", sline);
...
private:
    ...
    void parse_define() {
        ...
        if (at(Tok::StrLit) || at(Tok::Identifier) || at(Tok::Int)) {
            ...
            else if (name == "api.value.automove") {
                g_.api_value_automove = (v == "true" || v == "1");
            }
            else if (name == "api.token.constructor") {
                g_.api_token_constructor = (v == "true" || v == "1");
            }
            ...
            else if (name == "api.value.automove") g_.api_value_automove = true;
            else if (name == "api.token.constructor") g_.api_token_constructor = true;
        }
    }
...
private:
    ...
    void emit_cxx_header(Buf hdr) {
        ...
        if (g_.api_value_type == "variant") {
            // Tagged-union built on std::variant.  Each unique tag from
            // %token <T> / %type <T> becomes one alternative; std::monostate
            // covers the "no semantic value" case (untyped symbols).
            std::set<std::string> tags;
            for (auto& s : g_.syms) if (!s.type_tag.empty()) tags.insert(s.type_tag);
            hdr << "    private:\n";
            hdr << "    public:\n";
            hdr << "        typedef std::variant<std::monostate";
            for (auto& t : tags) hdr << ", " << t;
            hdr << "> semantic_type;\n";
        } else if (g_.api_value_type.empty() || g_.api_value_type == "int") {
    ...
    // Action-switch body.  In variant mode we also emplace the LHS type
    // into yyval at the head of each case (replacing the default
    // "yyval = yyvsp[1-yylen]" semantics that would copy a possibly
    // move-only type).  Bison documents this explicitly: with variants
    // the default $$ = $1 is NOT applied.
    void emit_cxx_action_switch(Buf out) {
        const bool variant = (g_.api_value_type == "variant");
        for (int i = 1; i < l_.n_rules(); i++) {
            const Production& p = l_.prod(i);
            const bool emplace_lhs = variant && !p.lhs_tag.empty();
            if (p.action.empty() && !emplace_lhs) continue;
            out << "    case " << i << ":\n";
            if (emplace_lhs) {
                out << "        (yyval).template emplace<" << p.lhs_tag
                    << ">();\n";
            }
            if (!p.action.empty())
    ...
    void emit_cxx_main(Buf out, Buf* hdr) {
        ...
        out << "    yylen = yyr2[yyn];\n";
        if (g_.api_value_type == "variant") {
            // Variant mode: emplace happens inside the action switch (per
            // rule, with the LHS type).  Skip the default $$ = $1 copy.
            out << "    yyval = semantic_type{};\n";
        } else {
            out << "    if (yylen) yyval = std::move(yyvsp[1 - yylen]);\n";
            out << "    else yyval = semantic_type{};\n";
        }
    ...
    string translate_action(const Production& p) {
        ...
        while (i < s.size()) {
            ...
            // For api.value.type=variant the semantic_type is std::variant.
            // Tagged references use std::get<T>(...) instead of (...).T;
            // untagged references stay as plain (yyval) / (yyvsp[N]).
            const bool variant = (g_.api_value_type == "variant");
            auto emit_typed = [&](const std::string& slot, const std::string& t) {
                if (variant) {
                    out += "std::get<"; out += t; out += ">(";
                    out += slot; out += ")";
                } else {
                    out += "("; out += slot; out += "."; out += t; out += ")";
                }
            };
            if (j < s.size() && s[j] == '$') {
                std::string slot = "yyval";
                std::string t = !tag.empty() ? tag : (g_.has_union ? p.lhs_tag : std::string());
                if (!t.empty()) emit_typed(slot, t);
                else { out += "(" + slot + ")"; }
                i = j + 1; continue;
            }
            if (j < s.size() && (ch_isdigit((unsigned char)s[j]) || s[j] == '-')) {
                ...
                int n = std::atoi(s.substr(j, k - j).c_str());
                int offset = n - eff_rhs_size;
                std::string slot = "yyvsp[" + std::to_string(offset) + "]";
                std::string t;
                if (!tag.empty()) t = tag;
                else if (n >= 1 && n <= (int)p.rhs_tags.size())
                    t = p.rhs_tags[n - 1];
                bool typed = !t.empty() && (variant || g_.has_union);
                std::string ref = typed
                    ? (variant ? "std::get<" + t + ">(" + slot + ")"
                               : "(" + slot + "." + t + ")")
                    : "(" + slot + ")";
                if (g_.api_value_automove) ref = "std::move(" + ref + ")";
                out += ref;
                i = k; continue;
            }
            if (j < s.size() && (ch_isalpha((unsigned char)s[j]) || s[j] == '_')) {
                ...
                if (found >= 0) {
                    int offset = (found + 1) - (int)p.rhs.size();
                    std::string slot = "yyvsp[" + std::to_string(offset) + "]";
                    std::string t = !tag.empty() ? tag : p.rhs_tags[found];
                    bool typed = !t.empty() && (variant || g_.has_union);
                    std::string ref = typed
                        ? (variant ? "std::get<" + t + ">(" + slot + ")"
                                   : "(" + slot + "." + t + ")")
